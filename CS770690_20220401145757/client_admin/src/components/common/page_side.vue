<template>
	<el-menu class="side el-menu-vertical-demo" default-active="/" @open="handleOpen" @close="handleClose"
		...
		<el-submenu index="user"
			...
			<el-menu-item index="/teacher_user/table" v-show="user_group == '管理员' || $check_action('/teacher_user/table')"><span>老师用户</span></el-menu-item>
			<el-menu-item index="/hospital_level_approver/table" v-show="user_group == '管理员' || $check_action('/hospital_level_approver/table')"><span>院级审批者</span></el-menu-item>
			...
			auth() {
				...
				for (var i = 0; i < tables.length; i++) {
					...
					if (o.path.indexOf("/table") !== -1) {
						...
						list.push("hospital_level_approver");
