		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','student_number') || $check_field('add','student_number') || $check_field('set','student_number')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="学生学号" prop="student_number">
					<el-input id="student_number" v-model="form_sub['student_number']" placeholder="请输入学生学号"
							  v-if="user_group === '管理员' || (form_sub['student_user_id'] && $check_field('set','student_number')) || (!form_sub['student_user_id'] && $check_field('add','student_number'))"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','student_name') || $check_field('add','student_name') || $check_field('set','student_name')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="学生姓名" prop="student_name">
					<el-input id="student_name" v-model="form_sub['student_name']" placeholder="请输入学生姓名"
							  v-if="user_group === '管理员' || (form_sub['student_user_id'] && $check_field('set','student_name')) || (!form_sub['student_user_id'] && $check_field('add','student_name'))"></el-input>
					<div v-else-if="$check_field('get','student_name')">{{form_sub['student_name']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','student_gender') || $check_field('add','student_gender') || $check_field('set','student_gender')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="学生性别" prop="student_gender">
					<el-select id="student_gender" v-model="form_sub['student_gender']"
						v-if="user_group === '管理员' || (form_sub['student_user_id'] && $check_field('set','student_gender')) || (!form_sub['student_user_id'] && $check_field('add','student_gender'))">
						<el-option v-for="o in list_student_gender" :key="o" :label="o" :value="o">
						</el-option>
					</el-select>
					<div v-else-if="$check_field('get','student_gender')">{{form_sub['student_gender']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','student_age') || $check_field('add','student_age') || $check_field('set','student_age')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="学生年龄" prop="student_age">
					<el-input id="student_age" v-model="form_sub['student_age']" placeholder="请输入学生年龄"
							  v-if="user_group === '管理员' || (form_sub['student_user_id'] && $check_field('set','student_age')) || (!form_sub['student_user_id'] && $check_field('add','student_age'))"></el-input>
					<div v-else-if="$check_field('get','student_age')">{{form_sub['student_age']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','affiliated_college') || $check_field('add','affiliated_college') || $check_field('set','affiliated_college')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="所属学院" prop="affiliated_college">
					<el-input id="affiliated_college" v-model="form_sub['affiliated_college']" placeholder="请输入所属学院"
							  v-if="user_group === '管理员' || (form_sub['student_user_id'] && $check_field('set','affiliated_college')) || (!form_sub['student_user_id'] && $check_field('add','affiliated_college'))"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','class_name') || $check_field('add','class_name') || $check_field('set','class_name')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="班级名称" prop="class_name">
					<el-input id="class_name" v-model="form_sub['class_name']" placeholder="请输入班级名称"
							  v-if="user_group === '管理员' || (form_sub['student_user_id'] && $check_field('set','class_name')) || (!form_sub['student_user_id'] && $check_field('add','class_name'))"></el-input>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 提交附加信息
			submit_sub(form_sub){
				if(form_sub["student_user_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?student_user_id=" + form_sub["student_user_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/student_user/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/student_user/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/student_user/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/student_user/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/student_user/view','get');
