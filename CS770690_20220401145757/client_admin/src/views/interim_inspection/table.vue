<template>
	<el-main class="bg">
			...
			<el-row>
				...
				<el-col :xs="24" :sm="10" :lg="8">
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/interim_inspection/table','add') || $check_action('/interim_inspection/view','add')" class="el-button el-button--default el-button--primary" to="./view?">添加
						</router-link>
						<el-button v-if="user_group == '管理员' || $check_action('/interim_inspection/table','del') || $check_action('/interim_inspection/view','del')" class="" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			...
			<el-table-column prop="student_number" label="学生学号"
			...
			<el-table-column prop="project_start_time" label="项目开始时间"
				v-if="user_group == '管理员' || $check_field('get','project_start_time')" min-width="200">
                <template slot-scope="scope">
                	{{ $toTime(scope.row["project_start_time"],"yyyy-MM-dd") }}
                </template>
			</el-table-column>
			<el-table-column prop="planned_completion_time" label="计划完成时间"
				v-if="user_group == '管理员' || $check_field('get','planned_completion_time')" min-width="200">
                <template slot-scope="scope">
                	{{ $toTime(scope.row["planned_completion_time"],"yyyy-MM-dd") }}
                </template>
			</el-table-column>
			<el-table-column prop="total_project_funds" label="项目经费总额"
				v-if="user_group == '管理员' || $check_field('get','total_project_funds')" min-width="200">
			</el-table-column>
			<el-table-column prop="research_progress" label="研究进展"
				v-if="user_group == '管理员' || $check_field('get','research_progress')" min-width="200">
			</el-table-column>
			<el-table-column prop="stage_results" label="阶段成果"
				v-if="user_group == '管理员' || $check_field('get','stage_results')" min-width="200">
			</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/interim_inspection/table','set') || $check_action('/interim_inspection/view','set') || $check_action('/interim_inspection/view','get') || $check_action('/结题验收/table','add') || $check_action('/结题验收/view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/interim_inspection/table','set') || $check_action('/interim_inspection/view','set') || $check_action('/interim_inspection/view','get')"
						:to="'./view?' + field + '=' + scope.row[field]"
						 size="small">
						<span>详情</span>
					</router-link>
					<!--跨表按钮-->
					<el-button class="el-button el-button--small is-plain el-button--default" style="margin: 5px !important;" size="small" @click="to_table(scope.row,'/final_acceptance/view')" v-if="user_group == '管理员' || $check_action('/final_acceptance/table','add') || $check_action('/final_acceptance/view','add')">
						<span>结题验收</span>
					</el-button>
				</template>
			</el-table-column>
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/interim_inspection/get_list?like=0",
				url_del: "~/api/interim_inspection/del?",
