	...
	<el-main class="bg">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','teacher_job_number') || $check_field('add','teacher_job_number') || $check_field('set','teacher_job_number')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="老师工号" prop="teacher_job_number">
					<el-input id="teacher_job_number" v-model="form_sub['teacher_job_number']" placeholder="请输入老师工号"
							  v-if="user_group === '管理员' || (form_sub['teacher_user_id'] && $check_field('set','teacher_job_number')) || (!form_sub['teacher_user_id'] && $check_field('add','teacher_job_number'))"></el-input>
					<div v-else-if="$check_field('get','teacher_job_number')">{{form_sub['teacher_job_number']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','teacher_name') || $check_field('add','teacher_name') || $check_field('set','teacher_name')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="老师姓名" prop="teacher_name">
					<el-input id="teacher_name" v-model="form_sub['teacher_name']" placeholder="请输入老师姓名"
							  v-if="user_group === '管理员' || (form_sub['teacher_user_id'] && $check_field('set','teacher_name')) || (!form_sub['teacher_user_id'] && $check_field('add','teacher_name'))"></el-input>
					<div v-else-if="$check_field('get','teacher_name')">{{form_sub['teacher_name']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','teacher_gender') || $check_field('add','teacher_gender') || $check_field('set','teacher_gender')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="老师性别" prop="teacher_gender">
					<el-select id="teacher_gender" v-model="form_sub['teacher_gender']"
						v-if="user_group === '管理员' || (form_sub['teacher_user_id'] && $check_field('set','teacher_gender')) || (!form_sub['teacher_user_id'] && $check_field('add','teacher_gender'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','teacher_age') || $check_field('add','teacher_age') || $check_field('set','teacher_age')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="老师年龄" prop="teacher_age">
					<el-input id="teacher_age" v-model="form_sub['teacher_age']" placeholder="请输入老师年龄"
							  v-if="user_group === '管理员' || (form_sub['teacher_user_id'] && $check_field('set','teacher_age')) || (!form_sub['teacher_user_id'] && $check_field('add','teacher_age'))"></el-input>
					<div v-else-if="$check_field('get','teacher_age')">{{form_sub['teacher_age']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','affiliated_college') || $check_field('add','affiliated_college') || $check_field('set','affiliated_college')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="所属学院" prop="affiliated_college">
					<el-input id="affiliated_college" v-model="form_sub['affiliated_college']" placeholder="请输入所属学院"
							  v-if="user_group === '管理员' || (form_sub['teacher_user_id'] && $check_field('set','affiliated_college')) || (!form_sub['teacher_user_id'] && $check_field('add','affiliated_college'))"></el-input>
					<div v-else-if="$check_field('get','affiliated_college')">{{form_sub['affiliated_college']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','teacher_certificate') || $check_field('add','teacher_certificate') || $check_field('set','teacher_certificate')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="老师证件" prop="teacher_certificate">
					<el-upload :disabled="disabledObj['teacher_certificate_isDisabled']" id="teacher_certificate" class="avatar-uploader" drag
						accept="image/gif, image/jpeg, image/png, image/jpg" action="" :http-request="upload_teacher_certificate"
						:show-file-list="false" v-if="user_group === '管理员' || (form_sub['teacher_user_id'] && $check_field('set','teacher_certificate')) || (!form_sub['teacher_user_id'] && $check_field('add','teacher_certificate'))">
						<img v-if="form_sub['teacher_certificate']" :src="$fullUrl(form_sub['teacher_certificate'])" class="avatar">
						<i v-else class="el-icon-plus avatar-uploader-icon"></i>
					</el-upload>
					<el-image v-else-if="$check_field('get','teacher_certificate')" style="width: 100px; height: 100px"
						:src="$fullUrl(form_sub['teacher_certificate'])" :preview-src-list="[$fullUrl(form_sub['teacher_certificate'])]">
...
<script>
	...
	export default {
		...
		data() {
			return {
				...
				disabledObj:{
					...
					"teacher_age_isDisabled": false,
					"affiliated_college_isDisabled": false,
					"teacher_certificate_isDisabled": false,
				},

				form_sub: {
					"teacher_job_number":'', // 老师工号
					"teacher_name":'', // 老师姓名
					"teacher_gender":'', // 老师性别
					"teacher_age":'', // 老师年龄
					"affiliated_college":'', // 所属学院
					"teacher_certificate":'', // 老师证件
				    "user_id": 0,
		...
		methods: {
			...
			// 提交附加信息
			submit_sub(form_sub){
				if(form_sub["teacher_user_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?teacher_user_id=" + form_sub["teacher_user_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/teacher_user/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/teacher_user/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/teacher_user/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/teacher_user/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/teacher_user/view','get');
