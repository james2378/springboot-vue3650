	...
	<el-main class="bg">
			...
			<el-row>
				...
				<el-col :xs="24" :sm="10" :lg="8">
					<el-form-item>
						...
						<router-link v-if="user_group == '管理员' || $check_action('/final_acceptance/table','add') || $check_action('/final_acceptance/view','add')" class="el-button el-button--default el-button--primary" to="./view?">添加
						</router-link>
						<el-button v-if="user_group == '管理员' || $check_action('/final_acceptance/table','del') || $check_action('/final_acceptance/view','del')" class="" type="danger" @click="delInfo()">删除</el-button>
					</el-form-item>
				</el-col>
		...
		<el-table :data="list" @selection-change="selectionChange" @sort-change="$sortChange" style="width: 100%" id="dataTable">
			...
			<el-table-column prop="contract_amount" label="合同金额"
				v-if="user_group == '管理员' || $check_field('get','contract_amount')" min-width="200">
			</el-table-column>
			<el-table-column prop="amount_in_place" label="到位金额"
				v-if="user_group == '管理员' || $check_field('get','amount_in_place')" min-width="200">
			</el-table-column>
			<el-table-column prop="actual_expenditure" label="实际支出"
				v-if="user_group == '管理员' || $check_field('get','actual_expenditure')" min-width="200">
			</el-table-column>
			<el-table-column prop="conclusion_annex" label="结题附件"
				v-if="user_group == '管理员' || $check_field('get','conclusion_annex')" min-width="200">
				<template slot-scope="scope">
					<a :href="$fullUrl(scope.row['conclusion_annex'])" target="_blank" style="color: rgb(64, 158, 255);">点击下载</a>
				</template>
			</el-table-column>
			<el-table-column prop="reply_ppt" label="答辩ppt"
				v-if="user_group == '管理员' || $check_field('get','reply_ppt')" min-width="200">
				<template slot-scope="scope">
					<a :href="$fullUrl(scope.row['reply_ppt'])" target="_blank" style="color: rgb(64, 158, 255);">点击下载</a>
				</template>
			</el-table-column>
			...
			<el-table-column prop="reason_for_conclusion" label="结题原因"
				v-if="user_group == '管理员' || $check_field('get','reason_for_conclusion')" min-width="200">
			</el-table-column>
			...
			<el-table-column fixed="right" label="操作" min-width="120" v-if="user_group == '管理员' || $check_action('/final_acceptance/table','set') || $check_action('/final_acceptance/view','set') || $check_action('/final_acceptance/view','get') || $check_action('//table','add') || $check_action('//view','add')" >


				<template slot-scope="scope">
					<router-link class="el-button el-button--small is-plain el-button--success" style="margin: 5px !important;"
					v-if="user_group == '管理员' || $check_action('/final_acceptance/table','set') || $check_action('/final_acceptance/view','set') || $check_action('/final_acceptance/view','get')"
	...
	export default {
		...
		data() {
			return {
				...
				showModal: false,
				// 获取数据地址
				url_get_list: "~/api/final_acceptance/get_list?like=0",
				url_del: "~/api/final_acceptance/del?",
