<template>
	<div class="diy_details_box diy_div_hospital_level_approver">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "hospital_level_approver_id",
					source_id: this.obj["hospital_level_approver_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/hospital_level_approver/set?hospital_level_approver_id=' + _this.obj["hospital_level_approver_id"], {
								praise_len
							}, (res) => {
								if(res.result){
