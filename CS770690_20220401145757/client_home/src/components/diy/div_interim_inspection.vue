<template>
	<div class="diy_details_box diy_div_interim_inspection">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "interim_inspection_id",
					source_id: this.obj["interim_inspection_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/interim_inspection/set?interim_inspection_id=' + _this.obj["interim_inspection_id"], {
								praise_len
							}, (res) => {
								if(res.result){
