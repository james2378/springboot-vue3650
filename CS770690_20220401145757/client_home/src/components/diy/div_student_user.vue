<template>
	<div class="diy_details_box diy_div_student_user">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "student_user_id",
					source_id: this.obj["student_user_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/student_user/set?student_user_id=' + _this.obj["student_user_id"], {
								praise_len
							}, (res) => {
								if(res.result){
