<template>
	<div class="diy_details_box diy_div_final_acceptance">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "final_acceptance_id",
					source_id: this.obj["final_acceptance_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/final_acceptance/set?final_acceptance_id=' + _this.obj["final_acceptance_id"], {
								praise_len
							}, (res) => {
								if(res.result){
