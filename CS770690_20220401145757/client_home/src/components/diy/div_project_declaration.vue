<template>
	<div class="diy_details_box diy_div_project_declaration">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "project_declaration_id",
					source_id: this.obj["project_declaration_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/project_declaration/set?project_declaration_id=' + _this.obj["project_declaration_id"], {
								praise_len
							}, (res) => {
								if(res.result){
