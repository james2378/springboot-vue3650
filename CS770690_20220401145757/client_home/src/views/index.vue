		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/project_rental_team/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_project_rental_team = !show_list_project_rental_team">
		                        <div class="switch_box">
		                            <span v-if="show_list_project_rental_team"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--项目租队推荐列表组件 -->
		                <list_project_rental_team v-if="show_list_project_rental_team" :list="list_project_rental_team" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_project_rental_team.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_project_rental_team" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_project_rental_team" :key="i" @click="to_details('project_rental_team',o,'project_rental_team_id')">
		                            <td v-for="(oj,n) in fields_project_rental_team">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/project_sharing/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_project_sharing = !show_list_project_sharing">
		                        <div class="switch_box">
		                            <span v-if="show_list_project_sharing"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--项目共享推荐列表组件 -->
		                <list_project_sharing v-if="show_list_project_sharing" :list="list_project_sharing" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_project_sharing.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_project_sharing" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_project_sharing" :key="i" @click="to_details('project_sharing',o,'project_sharing_id')">
		                            <td v-for="(oj,n) in fields_project_sharing">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
										<span v-else-if="oj.type && oj.type == 'UID' ">
											<span style="display: none">
...
<script>
	...
	import list_project_rental_team from "@/components/diy/list_project_rental_team.vue";
	import list_project_sharing from "@/components/diy/list_project_sharing.vue";
	...
	export default {
		mixins: [mixin],
		components: {
			list_project_rental_team,
			list_project_sharing,
			bar_title,
			list_article,
			swiper_img,
			swiper_notice,
		...
		data() {
			return {
				...
				show_list_project_rental_team: true,
				list_project_rental_team: [],
				vm_arr_project_rental_team: [
					"project_rental_team_id",
					"entry_name",
					"project_type",
					"student_job_number",
					"student_name",
				...
				show_list_project_sharing: true,
				list_project_sharing: [],
				vm_arr_project_sharing: [
					"project_sharing_id",
					"entry_name",
					"project_type",
					"affiliated_college",
		...
		methods: {
			...
			// 获取项目租队列表
			get_project_rental_team() {
				let url = "~/api/project_rental_team/get_list?";
				this.$get(url, {
					...
					if (json.result) {
						this.list_project_rental_team = json.result.list;
					}
				})
			},
			// 获取项目共享列表
			get_project_sharing() {
				let url = "~/api/project_sharing/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 8
				}, (json) => {
					if (json.result) {
						this.list_project_sharing = json.result.list;
					}
				})
			},
		...
		mounted() {
			this.get_project_rental_team();
			this.get_project_sharing();
			this.get_menu();
			this.get_slides();
			this.get_article();
			this.get_link();
		...
		computed: {
		    list_table_project_rental_team() {
		        var list = this.list_project_rental_team;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_project_rental_team.map((o) => {
		                // 第二个中括号是对象的属性
		    ...
		    list_table_project_sharing() {
		        var list = this.list_project_sharing;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_project_sharing.map((o) => {
		                // 第二个中括号是对象的属性
		                list_table[i][o] = list[i][o] || "";
		            });
		        }
