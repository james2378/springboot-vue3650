<template>
	<div class="diy_details page_project_rental_team" id="project_rental_team_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_project_rental_team'>
							<div_project_rental_team :obj="obj"></div_project_rental_team>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_project_rental_team from "@/components/diy/div_project_rental_team.vue";
	import mixin from "@/mixins/page.js";
	export default {
		mixins:[mixin],
		components:{
			div_project_rental_team
		},
		data(){
			return{
		...
		methods:{
			...
			get_praise(obj) {
				...
				this.$get("~/api/praise/count?", {
					...
					source_field: "project_rental_team_id",
					source_id: obj["project_rental_team_id"]
				}, (res) => {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "project_rental_team",
					source_field: "project_rental_team_id",
					source_id: this.obj["project_rental_team_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/project_rental_team/set?project_rental_team_id=' + _this.obj["project_rental_team_id"], {
								praise_len
							}, (res) => {
								if(res.result){
